...
class Solution {
    ...
    // TC: O(N) | SC: O(N)
    public ArrayList<Integer> bottomViewBFS(Node root) {
        ...
        // Maps HD -> last (bottommost) node value at that column
        ...
        while (!q.isEmpty()) {
            ...
            for (int i = 0; i < levelSize; i++) {
                ...
                // Always overwrite to keep the last (bottommost) node at each HD
    ...
    public void bottomViewDFS(Node root, int HD, int level, Map<Integer, Pair> mp) {
        if (root == null) return;

        // Update if HD is new OR current node is deeper (>= handles same-level tie -> latter wins)
        ...
        bottomViewDFS(root.left, HD - 1, level + 1, mp);
        bottomViewDFS(root.right, HD + 1, level + 1, mp);
    }
