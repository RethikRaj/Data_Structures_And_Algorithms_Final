class Solution {
    ...
    public int rob(int[] nums) {
        ...
        // int zerothHouseIncluded = nums[0] + f(nums, 2, n-2);
        // int zerothHouseExcluded = 0 + f(nums, 1, n-1);
        // return Math.max(zerothHouseIncluded, zerothHouseExcluded);

        // Top-down
        // int[] dp = new int[110];
        // Arrays.fill(dp, -1);
        // int zerothHouseIncluded = nums[0] + f_td(nums, 2, n-2, dp);
        // Arrays.fill(dp, -1); // !imp
        // int zerothHouseExcluded = 0 + f_td(nums, 1, n-1, dp);
        // return Math.max(zerothHouseIncluded, zerothHouseExcluded);

        // Bottom-up
        // int zerothHouseIncluded = nums[0] + f_bu(nums, 2, n-2);
        // int zerothHouseExcluded = 0 + f_bu(nums, 1, n-1);

        // return Math.max(zerothHouseIncluded, zerothHouseExcluded);

        // Bottom-up-optimised
        int zerothHouseIncluded = nums[0] + f_bu_optimised(nums, 2, n-2);
        int zerothHouseExcluded = 0 + f_bu_optimised(nums, 1, n-1);
